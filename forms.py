...
class ReaderRegisterForm(FlaskForm):
	username = StringField('Введите имя', 
		validators=[InputRequired('Введите имя'),
		Length(min=4, max=20, message='размер должен быть от 4 до 20 символов')])
	...
	confirm_password = PasswordField('Подтвердите пароль', 
		validators=[InputRequired('Подтвердите пароль'),
		Length(min=4, max=20, message='размер должен быть между 4 и 20 символами')])

# форма входа пользователя
class ReaderSigninForm(FlaskForm):
	readers = SelectField(
		choices=[(reader.username, reader.username) for reader in readers_list])
...
class AuthorBooksForm(FlaskForm):
	books = SelectMultipleField(
		choices=[(book.book_name, book.book_name) for book in book_list])
